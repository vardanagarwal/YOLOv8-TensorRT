...
def associate(
    ...
    detections = np.array(
        [[value for value in row if isinstance(value, float)] for row in detections]
    )
    trackers = np.array(
        [[value for value in row if isinstance(value, float)] for row in trackers]
    )
    ...
    valid_mask[np.where(previous_obs[:, 3] < 0)] = 0
