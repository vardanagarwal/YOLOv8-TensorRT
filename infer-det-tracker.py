...
tracker = OCSort(det_thresh=0.25)
...
def main(args: argparse.Namespace) -> None:
    ...
    for i in range(0, len(images), BATCH_SIZE):
        ...
        for result in processed_results:
            ...
            tracked_detection = []
            for track in tracks:
                if len(track) == 8:  # Avoid garbage value
                    bbox = track[:4].tolist()
                    tracked_detection.append(
                        {
                            "object_id": int(track[4]),
                            "confidence": round(float(track[5]), 2),
                            "name": detections_classes[float(track[6])],
                            "x1": bbox[0],
                            "y1": bbox[1],
                            "x2": bbox[2],
                            "y2": bbox[1],
